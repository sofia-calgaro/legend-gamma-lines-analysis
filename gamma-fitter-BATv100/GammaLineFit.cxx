  ...
  BCEngineMCMC::Precision precision ) 
{
  ...
  for(int i=0;i<fBkgFunction->GetNpar();i++)                               // background parameters
   log << std::setw(15) << left  << Form(" %s:",fBkgFunction->GetParName(i))
       << std::setw(15) << right << Form("(%.1f+-%.1f)",bestFitPars[i],bestFitParErrors[i])
       << " cts" << (i>0&&backgroundType!=kStep?"/keV":"") << (i>1?Form("^%d",i):"")       << "\n";
  for(unsigned int i=0;i<lines.size();i++) {                                     // line parameters
   log << "--------------"                                                                 << "\n";
   log << std::setw(15) << left  << Form(" %s:",fFitFunction->GetParName(nBkgPars+3*i+0))
       << std::setw(15) << right << Form("(%.1f+-%.1f)",bestFitPars[nBkgPars+3*i+0],bestFitParErrors[nBkgPars+3*i+0])
       << Form(" keV (nom. %.1f keV)",lines.at(i))                                         << "\n";
   log << std::setw(15) << left  << Form(" %s:",fFitFunction->GetParName(nBkgPars+3*i+1))
       << std::setw(15) << right << Form("(%.2f+-%.2f)",bestFitPars[nBkgPars+3*i+1],bestFitParErrors[nBkgPars+3*i+1])
       << Form(" keV (nom. %.1f keV)",fRes->Eval(lines.at(i)))                             << "\n";
   log << std::setw(15) << left << Form(" %s:",fFitFunction->GetParName(nBkgPars+3*i+2));
  //************************************************************************
  ...
  //************************************************************************
   ...
   log << " p-value:      " << std::setw(15) << fHistFitter->GetPValue()                        << "\n";    
   log << "------------------------------------------------------------"                   << "\n";
  //-----------------------------------------------------------------------------------------------
  log.close();
  ...
  fHistFitter->PrintKnowledgeUpdatePlots(Form("%s/%s.%s.pdf",fOutputDir.Data(),GetName(),name.Data()), 2,2);
